...
export const calculateStats = (data: RenovationData[]) => {
  ...
  // Économies moyennes par surface (utiliser la moyenne de Categorie)
  data.forEach(row => {
    const economieAnnuelle = row.Economie_Prix_Elec_An_Global + row.Economie_Prix_Gaz_An_Global;
    // Calculer la surface moyenne depuis Categorie (ex: "50-60" -> 55)
    const [min, max] = row.Categorie.split('-').map(Number);
